...
const SUBSCRIPTION_TIERS = {
  starter:    { price_usd: 20.0, calls_per_day: 10000, label: 'Starter' },
  pro:        { price_usd: 99.0, calls_per_day: 100000, label: 'Pro' },
...
async function emitSpectralReceipt({ event_type, did, amount_usd, tool_name, tx_hash, metadata }) {
  ...
  try {
    ...
    console.warn('[abtest] receipt emit failed (non-fatal):', _.message || _);
  }
}
...
app.post('/v1/subscription', async (req, res) => {
  ...
  if (tier !== 'enterprise' && !tx_hash) {
    return res.status(402).json({
      error: 'payment_required',
      x402: {
        type: 'x402', version: '1', kind: 'subscription_abtest',
  ...
  await emitSpectralReceipt({
    ...
    tx_hash: tx_hash || null,
    metadata: { tier, service: 'Hive A/B Test', expires_ms: record.expires_ms },
  });
  ...
  return res.json({
    ...
    partner_attribution: 'A/B test orchestration — Spectral-signs stat-sig conclusions for tamper-evident audit. Complements Optimizely, Split.io.',
    brand: '#C08D23',
